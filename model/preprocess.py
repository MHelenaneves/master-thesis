...
def band_pass_filter(data_df, sampling_rate, bp_cutoff, order, channels=['X', 'Y', 'Z']):
    ...
    critical_frequency_low = bp_cutoff[0]* 2.0 / sampling_rate
    critical_frequency_high=bp_cutoff[1]* 2.0 / sampling_rate
    ...
    [b, a] = signal.butter(N=order, Wn=critical_frequency_low, btype='highpass', analog=False)
    [d,c] = signal.butter(N=order, Wn=critical_frequency_high, btype="lowpass")

    # Apply filter to raw data
    bp_filtered_data1 = signal.filtfilt(b, a, data, axis=0)
    bp_filtered_data = signal.filtfilt(d, c, data, axis=0)
